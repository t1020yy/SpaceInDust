...
FILE_NAME = 'modeling_results_ 2023-11-04_00-44-47.pickle'
...
# plt.xlabel('cams_trans_vec_x (mm)', fontsize = 20)
# plt.ylabel('angle_between_cameras (°)', fontsize = 20)
# plt.title('angle_err', fontsize = 20)
...
# plt.xlabel('cams_trans_vec_x (mm)', fontsize = 20)
# plt.ylabel('angle_between_cameras(°)', fontsize = 20)
# plt.title('velocity_err', fontsize = 20)
# plt.xticks(fontsize = 20)  
# plt.yticks(fontsize = 20)
# plt.grid()
# plt.show()
...
mean_angle_err = [np.mean(angle_err[diameter == d]) for d in unique_diameters]
mean_velocity_err = [np.mean(velocity_err[diameter == d]) for d in unique_diameters]
std_dev_angle_err = [np.std(angle_err[diameter == d]) for d in unique_diameters]
std_dev_velocity_err = [np.std(velocity_err[diameter == d]) for d in unique_diameters]
...
# #计算Boxplot图
data_a = [angle_err[diameter == d] for d in unique_diameters]
data_v = [velocity_err[diameter == d] for d in unique_diameters]
...
plt.boxplot(data_a, labels=unique_diameters)
plt.xlabel('Partice diameter (mm)', fontsize = 20)
plt.ylabel('Angle error (°)', fontsize = 20)
...
plt.boxplot(data_v, labels=unique_diameters)
plt.xlabel('Partice diameter (mm)', fontsize = 20)
plt.ylabel('Velocity error (m/s)', fontsize = 20)
plt.xticks(rotation=45,fontsize = 15)  
plt.yticks(fontsize = 15)
